...
## v0.6 — 卡片体验 + 排序优化 ✅ 2026-04-29

**目标**: 精简卡片信息展示，统一排序逻辑，完善键盘导航的自动滚动。

> Done when: 卡片只显示必要信息(无分秒/无日期)；Pin 不再影响排序，全部按时间顺序；切换 Pinboard 也按时间排；键盘左右切换时不可见卡片自动滚入视口。

- [x] **F-54** 去除卡片下侧的分秒时间显示
- [x] **F-55** 去除卡片上侧的日期显示
- [x] **F-56** 统一排序为纯时间顺序(Pin 不再置顶；选择 Pinboard 时也按被 pin 卡片的时间顺序排)
- [x] **F-57** 键盘左右切换自动滚动(当选中卡片移出可见区域时，自动将 ScrollView 滚动到选中卡片位置)
- [x] **F-58** 卡片双击粘贴(双击卡片与按 Enter 键效果一致，触发粘贴到前台应用)

---

## v0.7 — 性能优化 + Liquid Glass 主题 ✅ 2026-04-29

**目标**: 优化卡片切换性能，排查偏好设置功能完整性，引入 macOS 26 Liquid Glass API 重构主题系统。

> Done when: 键盘切换卡片无明显延迟(≤30ms)；偏好设置所有控件生效；主题使用 Liquid Glass 实现真正的投射效果。

- [x] **F-59** 卡片切换性能日志(selectNext/selectPrevious 加耗时打点，定位瓶颈)
- [x] **F-60** 卡片切换性能优化(根据日志结果优化 displayClips 计算缓存/动画/渲染)
- [x] **F-61** 菜单栏图标打开偏好设置(排查 BUG-017 修复后是否回归，确保菜单栏点击可打开 Preferences 窗口)
- [x] **F-62** 偏好设置功能全量排查(glass blur 滑块未实际生效、历史上限切换是否立即裁剪、所有 Appearance 控件验证)
- [x] **F-63** Liquid Glass 主题重构(使用 macOS 26 Liquid Glass API 替代当前渐变 + .ultraThinMaterial 方案，实现面板后方桌面背景投射的液态玻璃效果)
- [x] **F-64** 主题 Liquid Glass 适配(移除 Warm/Cool 主题，macOS 26+ 使用 Liquid Glass，旧系统使用 Mono 渐变)

---

## v0.5 — 用户实测修复 ✅ 2026-04-29

**目标**: 修复用户实测发现的 13 个 bug，增强文件检测和 Pin 功能。

> Done when: 设置页正常渲染；Finder 复制各类文件/图片正确识别；Pin 功能完整可用；卡片布局稳定。

- [x] **F-65** Create Pinboard 快捷创建：右键 Pin 菜单和 Pin Picker 底部新增 "Create Pinboard" 选项，自动命名 "Pinboard 0/1/2…" 并去重
- [x] **F-66** SwiftData 迁移自动降级：ModelContainer 创建失败时删除不兼容数据库重建，loadFromContext 错误日志输出

---

## v0.8.1 — 主题色调整 ✅ 2026-04-29

**目标**: 将主题色由橙色调整为紫色，统一 Sidebar / Card / Preferences 三处视觉。

- [x] **F-69** 主题色由橙色 `#F4A261`(深色 `#E76F51`) 调整为紫色 `#7870F2`(深色 `#5048D9`)；Pinboard 默认 accent 同步更新
